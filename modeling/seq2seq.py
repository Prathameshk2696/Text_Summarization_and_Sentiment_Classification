...
from rnn import rnn_encoder
from rnn import rnn_decoder
...
class seq2seq(nn.Module):
    def __init__(self, config, use_attention = True, encoder = None, decoder = None):
        ...
        if decoder is not None:
            ...
            self.decoder = rnn_decoder(config, embedding=tgt_embedding, use_attention=use_attention)
        self.log_softmax = nn.LogSoftmax(dim=-1)
        self.use_cuda = config.use_cuda
        self.config = config
        self.criterion = nn.CrossEntropyLoss(ignore_index = make_vocab.PAD, size_average=False)
        if config.use_cuda:
            self.criterion.cuda()
    ...
    def compute_loss(self, scores, targets):
        if self.config.max_split > 0:
            ...
            for i, (out_t, targ_t) in enumerate(zip(outputs_split, targets_split)):
                ...
                num_total_t = targ_t.ne(make_vocab.PAD).data.sum()
                loss += loss_t.data[0]
    ...
    def sample(self, src, src_len):
        ...
        bos = Variable(torch.ones(src.size(0)).long().fill_(make_vocab.BOS), volatile=True)
        src = src.t()
